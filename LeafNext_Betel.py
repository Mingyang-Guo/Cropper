"""
LeafConvNeXt: A ConvNeXt-based model for plant disease classification.
Architecture based on ConvNeXt (Large-kernel depthwise conv, LayerNorm, MLP head):contentReference[oaicite:0]{index=0}:contentReference[oaicite:1]{index=1}.
Includes global average pooling and a classification head:contentReference[oaicite:2]{index=2}.
Integrates LayerCAM for visualization of important regions:contentReference[oaicite:3]{index=3}.
"""
import torch
...
class LeafConvNeXt(nn.Module):
    def __init__(self, num_classes=2, depths=[3, 3, 9, 3], dims=[96, 192, 384, 768]):
        super().__init__()
...
class BalancedDataset(Dataset):
    def __init__(self, root_dir, transform=None, target_transform=None, samples_per_class=500, balance=True):
        """
        root_dir: path to dataset organized by class subdirectories.
        transform: torchvision transforms to apply to each image.
        samples_per_class: number of samples per class for balancing.
        balance: whether to balance classes (True for training, False for validation/test).
        """
...
train_dir = "/home/guest/mingyang/Betel2/Train"
val_dir = "/home/guest/mingyang/Betel2/Valid"
test_dir = "/home/guest/mingyang/Betel2/Test"
...
# 3. Loss function: Label Smoothing Loss
class LabelSmoothingLoss(nn.Module):
    def __init__(self, epsilon: float = 0.1):
        """
        Label smoothing helps prevent overconfidence by softening labels:contentReference[oaicite:7]{index=7}.
        """
...
def load_pretrained_convnext(num_classes=2):
...
model = load_pretrained_convnext(num_classes=2).to(device)
...
# Learning rate schedule: 20-epoch linear warmup, then cosine annealing:contentReference[oaicite:8]{index=8}.

# 5. Visualization: LayerCAM integration
# Example function to generate and save a CAM heatmap for one input image
"""
def generate_layer_cam(model, input_tensor, target_class, layer_name="features.5"):

    Generate LayerCAM heatmap for a given input and target class.
    layer_name: name of the layer to visualize (e.g., last convolutional block)

    # Select the target layer for CAM (example: last stage of ConvNeXt)
    target_layer = dict([*model.backbone.features.named_modules()])[layer_name]
    cam = LayerCAM(model=model.backbone, target_layers=[target_layer], use_cuda=torch.cuda.is_available())
    grayscale_cam = cam(input_tensor=input_tensor.unsqueeze(0), targets=[ClassifierOutputTarget(target_class)])
    grayscale_cam = grayscale_cam[0]  # batch size 1
    # Overlay CAM on image (assuming input_tensor normalized)
...
# 6. Training and evaluation loop (simplified; specifics depend on actual dataset paths)
...
def evaluate_with_metrics(model, loader, criterion):
    ...
    metrics['auroc'] = roc_auc_score(all_true, np.array(all_probs)[:, 1])
...
history = {
    ...
    'train_precision': [], 'train_recall': [], 'train_f1': [], 'train_specificity': [], 'train_auroc': [],
    'val_precision': [], 'val_recall': [], 'val_f1': [], 'val_specificity': [], 'val_auroc': []
}
best_val_loss = float('inf')

best_model_path = "betel100.pth"
...
for epoch in tqdm(range(num_epochs)):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), 'betel100.pth')
...
def plot_confusion_matrix(model, test_loader):
    ...
    class_labels = ['cabbage_healthy', 'cabbage_CP_deficiency']
...
model.load_state_dict(torch.load('betel100.pth'))
